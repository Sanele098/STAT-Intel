...
export default function PublicDashboardPage() {
  ...
  async function confirmEscalation() {
    ...
    setLoading(true)
    try {
      const response = await fetch('/api/public/ask', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: pendingEscalation.question, caseId: pendingEscalation.caseId, allowEscalation: true }) })
      const result = await response.json()
      ...
      setError(err instanceof Error ? err.message : 'This enquiry could not be escalated.')
  ...
  async function askQuestion(event?: FormEvent, presetQuestion?: string) {
    ...
    setError('')
    try {
      const response = await fetch('/api/public/ask', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: asked, caseId: conversation?.caseId, sessionId, allowEscalation: false }) })
      const result = await response.json()
      ...
      setError(err instanceof Error ? err.message : 'We could not process that enquiry.')
    } finally { setLoading(false) }
  }
