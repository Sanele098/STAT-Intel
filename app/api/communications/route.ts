...
function toCase(item: any, sources: any[]) {
  ...
  const sourceMatches = sources.filter((source) => `${source.title} ${source.topic} ${source.content ?? ''}`.toLowerCase().split(/\s+/).some((term) => term.length > 4 && question.toLowerCase().includes(term)))
...
export async function GET() {
  ...
  const [{ data: enquiries, error: enquiryError }, { data: sources, error: sourceError }] = await Promise.all([
    supabase.from('public_queries').select('id,case_reference,question,answer,status,created_at,updated_at').order('created_at', { ascending: false }),
    supabase.from('statsa_knowledge_sources').select('id,title,topic,content,source_date,source_url').order('source_date', { ascending: false }),
  ])
